...
class Info(commands.Cog):
    ...
    async def bot(self, ctx):
        ...
        embedve = discord.Embed(
            title="KeithBot Info", description="Lol", color=0x9370DB)
        embedve.add_field(
    ...
    async def help(self, ctx):
        embedvar = discord.Embed(title="Help Commands",description="Lol", color=0x00ff00)
        ...
        embedvar.add_field(name='?pc ***action***', value='Use PC (if bought)')
        embedvar.add_field(name='?uno', value='Use Uno Reverse (if bought)', inline=False)
